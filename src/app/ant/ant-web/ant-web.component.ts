...
@Component({
  selector: 'app-ant-web',
  templateUrl: './ant-web.component.html',
  styleUrls: ['./ant-web.component.scss'],
  
})
export class AntWebComponent implements OnInit {
  antWebData: any = []
  isDragging = true;
  ...
  url = this.router.url;
  popup : any;
  constructor(private sanitizer: DomSanitizer ,private antService : AntService, private faqservice : FaqService , 
    private seoService : SeoService , private meta: Meta , private title: Title , 
    private router : Router , private popupservice: PopupService , public dialog: MatDialog) { 
  ...
  getAntData() {
    ...
    this.antService.fetchAntData(data).subscribe((response) => {
      ...
      this.antWebData = response[0] == 'No Data'?[]:response;
