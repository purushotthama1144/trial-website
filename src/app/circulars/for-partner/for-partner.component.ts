...
@Component({
  selector: 'app-for-partner',
  templateUrl: './for-partner.component.html',
  styleUrls: ['./for-partner.component.scss'],
  
})
export class ForPartnerComponent implements OnInit {
  ...
  refreshDataSource(){
    this.dataSource = new MatTableDataSource(this.partner);
  ...
  applyFilter(event: any) {
    this.filterData = this.partner.filter((data)=> new Date(data.date).toLocaleDateString() == new Date(event.value).toLocaleDateString())
  ...
  getcircularData(){
    const data = {
      "circular_category":"For Partner"
    }
    this.circularsService.fetchCircularData(data).subscribe((response) => {
      this.partner = response = response[0] == 'No Data'?[]:response.reverse();
       // date sort decending order
       this.partner.sort((val1:any, val2:any)=> {
