...
@Component({
  selector: 'app-tender-period-contracts',
  templateUrl: './tender-period-contracts.component.html',
  styleUrls: ['./tender-period-contracts.component.scss'],
  
})
export class TenderPeriodContractsComponent implements OnInit {
  tendercontract: any
  ...
  getTenderData() {
    const data = {
      "rms_calculator_type" : "Tender Period Contracts"
    }
    this.rmsService.getRmsliveData(data)
      .subscribe((response: any) => {
       
        this.tendercontract = response
      }, (error) => {
