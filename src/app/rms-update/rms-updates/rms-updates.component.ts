...
@Component({
  selector: 'app-rms-updates',
  templateUrl: './rms-updates.component.html',
  styleUrls: ['./rms-updates.component.scss'],
  
})
export class RmsUpdatesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getBanSymbolData()
    this.getSeoData()
  }

  getBanSymbolData() {
    const data = {
      "rms_calculator_type" : "RMS Update"
    }
    this.rmsService.getRmsliveData(data)
      .subscribe((response: any) => {
    
        this.rmsupdate = response
      }, (error) => {
